...
def isValid(phrase):
    for x in range(len(phrase) - 1):
        if phrase[x].lower() == phrase[x + 1].lower() and (
            (phrase[x].islower() and phrase[x + 1].isupper())
            or (phrase[x].isupper() and phrase[x + 1].islower())
        ):
            return False
    return True
...
def getRagas(message, threshold=0.5, joiner=" or "):  # guessing code
    ...
    for guess in keys:
        ...
        if SAPTAKS[guess] / tri(len(message)) >= threshold and guess in heapq.nlargest(
            1, SAPTAKS, SAPTAKS.get
        ):
